...
int source_to_crl(struct s2c* arg, double L)
{
    ...
    double r;			        //distance to source-point
    double A;				//Amplitude
    double ph, prevph, phdif;		//phase, previousphase, phase difference
  ...
  /*Delta-N loop, max phase difference*/
    ...
    printf("%e %f",delta, phdif);
 
    /*//Check if N meets the minimum to have the delta required
    if (N<Nmin)
    {
        N=Nmin; 
	delta=(*L)/N;
        fprintf(stderr, "warning: N value changed to Nmin=%d in %s .\n", Nmin, __FUNCTION__);
    }*/
     ...
     * change number of points N so that phase shift between two points << 2pi, (pi/10 suggested)
